...
**Quick Impl** (`@.opencode/instincts/quick-impl.md`):
- Não existem artefatos `analysis_*.json` nem `plan_*.json` — não tente lê-los
- Vá direto à exploração do código fonte
- Não execute sub-agentes de fase (analyzer/planner/verifier)
- Responda diretamente ao usuário (markdown conciso), não escreva em `@.agentic/memory/`
...
1. **Avaliação de Conhecimento**: Se a solução envolveu um erro complexo ou um padrão de código recorrente, pergunte: "Deseja catalogar esta solução em `@.agentic/brain/wiki/troubleshooting.md`?".
...
> **Nota**: Este protocolo aplica-se APENAS a interações conversacionais durante correções/implementações. NÃO se aplica a saídas de fase (analyzer/planner/implementer/verifier). Saída de fase é exclusivamente JSON em `@.agentic/memory/` — zero prosa.
...
4. Após escrever artefato JSON de fase em `@.agentic/memory/`, a resposta ao usuário DEVE ser vazia ou no máximo "Concluído." — sem sumários, markdown, tabelas, explicações ou qualquer texto adicional
...
Regras:
- Antes de responder a perguntas sobre arquitetura ou código-fonte, leia graphify-out/GRAPH_REPORT.md para obter informações sobre nós de conhecimento e estrutura da comunidade.
- Se graphify-out/wiki/index.md existir, navegue até ele em vez de ler os arquivos brutos.
- Após modificar arquivos de código nesta sessão, execute `graphify update .` para manter o grafo atualizado (somente AST, sem custo de API).
